...
```{r correct-swap, echo=FALSE, eval=FALSE}
sampleNames(eset)[match(swapLS, table = sampleNames(eset))] <- rev(swapLS)
pData(eset)[swapLS, "Cell.population.sorted"] <- rev(pData(eset)[swapLS, "Cell.population.sorted"])
save(eset, file = file.path(workDir, "output/joana.eset.RData"))
```
